...
- **User 選單**: 7 項 → 5 項
  - 移除「查詢代碼表」(Admin 專屬)
  - 移除「查詢動物趨勢」(Admin 專屬，整合至健康監控)
- **Admin 選單**: 13 項 → 9 項
  - 合併「批量異常掃描」+「高風險動物」+「動物趨勢」→「健康監控」
  - 合併「庫存報表」+「庫存進貨」→「庫存管理」
  - 合併「員工管理」+「管理員工證照」→「員工管理」
  - 移除「查詢代碼表」

### 新增功能
- **密碼登入**: 員工需輸入密碼登入 (預設: zoo123)
- **忘記密碼**: 登入畫面輸入 `f` 可查詢密碼 (展示用)
- **員工管理**: 新增/停用員工、變更角色、管理證照
- **飲食管理**: 設定各物種可食用的飼料
- **長連線架構**: Client-Server 改為持久連線，提升效能
- **返回功能**: 所有輸入步驟皆可輸入 `b` 返回上一層

### 資料庫變更
- `employee` 表新增 `password_hash` 欄位
- 新增 `animal_diet` 表 (物種-飼料對應)
